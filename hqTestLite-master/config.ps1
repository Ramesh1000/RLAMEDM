...
If (!$MedmProcessAgentPath) { $Global:MedmProcessAgentPath = "C:\Program Files (x86)\Markit Group\Markit EDM_10_2_3_2\CadisProcessAgent.exe" }
...
If (!$ReportFolder) { $Global:ReportFolder = "C:\Ramesh\hqLite\Results" }
...
switch ($ActiveEnvironment) {
    "DEV" {
        $Global:EnvMedmDbServer = "VCDVAAMSMEDM01.TESTROYALLONDONGROUP.COM"
        $Global:EnvMedmDbName = "MarkitEDM_DEV_20180706"
#        $Global:MSCI LQM Out = "\\testroyallondongroup.com\shares\EDM_DEV\Markit\OUT\MSCI\LQM"
#        $Global:MSCI LQM Out Working = "\\netshare\DEV\bbg\security\IN"
        break
    }
